...
if __name__ == "__main__":
    ...
    engine = create_engine('postgresql://postgres:postgres@localhost:5432/dwh_final')

    conf_hadoop = connection.param_config("hadoop")["ip"]
    client = InsecureClient(conf_hadoop)

    with client.read(f'/final_project/{time}/orders_{time}.csv', encoding='utf-8') as writer:
        df = pandas.read_csv(writer)
    df.to_sql("dwh_dim_orders", engine, if_exists='replace', index=False)
